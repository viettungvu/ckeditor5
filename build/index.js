...
function createEditor(editorId,  placehoder = "", data = "") {
  const editorContent = $("#" + editorId).find(".question-editor__content");
  return XClassicEditor.create(editorContent[0], {
    controlConfig: {
      katexRenderOptions: {
        output: "html",
        throwOnError: false,
    ...
    placeholderConfig: {
      types: ["ho_ten_ung_vien", "vi_tri_ung_tuyen", "tin_tuyen_dung"],
    },
    fontSize: {
      options: [8, 9, 10, 11, 12, 13, 14, 16, 20],
    },
    table: {
    ...
    .then((editor) => {
      ...
      //CodeDesign.XMEditor.Inspector.attach(editor);
...
function init() {
  ...
  createEditor("editor-cau-hoi", "Nhập nội dung câu hỏi", content);
...
function bindActionEditor() {
  ...
  editorKeys.forEach((editorId) => {
    ...
    $("#collapseDragEquationQuestion" + editorId).on("show.bs.collapse", (e) => {
      ...
      $("#" + editorId)
        .find(".question-group__left")
        .addClass("select-box__opened");
    });
    $("#collapseDragEquationQuestion" + editorId).on("hide.bs.collapse", (e) => {
      const editorId = $(e.target).data("editor-id");
      $("#" + editorId)
        .find(".question-group__left")
        .removeClass("select-box__opened");
    });
