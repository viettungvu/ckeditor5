...
export default class ControlEditing extends Plugin {
    ...
    createControlView(modelItem, viewWriter) {
        ...
        if (modelItem != null) {
            ...
            var containerName = cfig.forceOutputType ? cfig.forceOutputType : (display ? 'div' : 'span');
            if (type === ControlType.LUA_CHON) {
                try {
                    let optionsView;
                    let displayElement = viewWriter.createRawElement('span', {}, function (domElement) {
                        domElement.innerHTML = 'Chọn';
                    });
                    if (values.length == 0) {
                        optionsView = viewWriter.createRawElement('span', { class: 'custom-option', value: 'option1', 'data-equation': 'option1' }, 'Option 1');
                    }
                    else {
                        optionsView = values.map((v, index) => viewWriter.createRawElement('span', { class: 'custom-option', value: v.value, 'data-equation': v.equation }, function (domElement) {
                            Utils.renderEquationControl(v.equation, domElement, cfig.katexRenderOptions);
                            return domElement;
                        }));
                    }
                    cardView = viewWriter.createContainerElement(containerName, {
                        class: classList, style: styles, 'data-type': type, 'data-id': id
                    },
                        viewWriter.createContainerElement('span', { class: 'xcustom-select-wrapper', onclick: 'xSelectOptionCustom(this)' },
                            viewWriter.createContainerElement('span', { class: 'xcustom-select' }, [
                                viewWriter.createContainerElement('input', { type: 'hidden', name: 'ANS_LUA_CHON_' + id, value: '' }),
                                viewWriter.createContainerElement('span', { class: 'xcustom-select__trigger' }, [
                                    displayElement,
                                    viewWriter.createContainerElement('span', { class: 'arrow' })
                                ]),
                                viewWriter.createContainerElement('span', { class: 'xcustom-options' }, optionsView)
                            ]),
                ...
                return viewWriter.createContainerElement(containerName, {
                    class: classList, style: styles, 'data-type': type, 'data-id': id
                ...
                try {
                    ...
                    if (values.tuSo == '') {
                        ...
                        childrenElements.push(viewWriter.createContainerElement('span', { class: 'tu-so', value: values.tuSo }, viewWriter.createRawElement('span', {}, function (domElement) {
                            Utils.renderEquationControl(values.tuSo, domElement, cfig.katexRenderOptions);
                            return domElement;
                        })));
                    }
                    if (values.mauSo == '') {
                        ...
                        childrenElements.push(viewWriter.createContainerElement('span', { class: 'mau-so', value: values.mauSo }, viewWriter.createRawElement('span', {}, function (domElement) {
                            Utils.renderEquationControl(values.mauSo, domElement, cfig.katexRenderOptions);
                            return domElement;
                        })));
                    }
                    cardView = viewWriter.createContainerElement(containerName, { class: classList, style: styles, 'data-type': type, 'data-id': id }, viewWriter.createContainerElement('span', { class: 'frac frac-input' }, childrenElements));
                } catch (error) {
                ...
                try {
                    ...
                    if (values.soBiChia == '') {
                        ...
                        firstChilds.push(viewWriter.createContainerElement('span', { class: 'so-bi-chia', value: values.soBiChia }, viewWriter.createRawElement('span', {}, function (domElement) {
                            Utils.renderEquationControl(values.soBiChia, domElement, cfig.katexRenderOptions);
                            return domElement;
                        })));
                    }
                    if (values.soChia == '') {
                        ...
                        secondChilds.push(viewWriter.createRawElement('span', { class: 'so-chia', value: values.soChia }, function (domElement) {
                            Utils.renderEquationControl(values.soChia, domElement, cfig.katexRenderOptions);
                            return domElement;
                        }));
                    }
                    ...
                    if (values.thuongSo == '') {
                        ...
                        secondChilds.push(viewWriter.createRawElement('span', { class: 'thuong-so', value: values.thuongSo }, function (domElement) {
                            Utils.renderEquationControl(values.thuongSo, domElement, cfig.katexRenderOptions);
                            return domElement;
                        }));
                    }
                    if (values.soDu == '') {
                        ...
                        firstChilds.push(viewWriter.createRawElement('span', { class: 'so-du', value: values.soDu }, function (domElement) {
                            Utils.renderEquationControl(values.soDu, domElement, cfig.katexRenderOptions);
                            return domElement;
                        }));
                    }

                    cardView = viewWriter.createContainerElement(containerName, { class: classList, style: styles, 'data-type': type, 'data-id': id }, viewWriter.createContainerElement('span', { class: 'division' }, [
    ...
    getControlDataFromViewElement(viewElement) {
        if (viewElement != null) {
            try {
                ...
                let filteredArray = BackgroundColorClass.filter(value => elementClasses.includes(value));
                let classList = '';
                if (filteredArray.length == 0) {
                    classList += 't-control ' + BackgroundColorClass[controlId % BackgroundColorClass.length];
                } else {
                ...
                if (controlType === ControlType.LUA_CHON) {
                    try {
                        let customOptions = Array.from(viewElement.getChild(0).getChild(0).getChild(2).getChildren());
                    ...
                    try {
                        let phanSoElement = children.find(e => e.is('element', 'span') && e.hasClass('frac', 'frac-input'));
                        let phanSoChild = Array.from(phanSoElement.getChildren());
                        let tuSoElement = phanSoChild.find(element => (element.is('element', 'span')) && element.hasClass('tu-so'));
                        let mauSoElement = phanSoChild.find(element => (element.is('element', 'span')) && element.hasClass('mau-so'));
                        return {
                            id: controlId,
                            value: {
                    ...
                    try {
                        let divisionElement = children.find(e => e.is('element', 'span') && e.hasClass('division'));
